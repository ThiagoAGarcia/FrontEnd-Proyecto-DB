    ...
    <>
      <div className="w-full overflow-hidden">
        <AnimatePresence mode="wait">
          {!selectedBuilding && (
            <motion.div
              ...
              exit="exit"
              animate="initial"
              className="grid lg:grid-cols-3 xl:grid-cols-4 sm:grid-cols-2 grid-cols-1 gap-6 place-items-center overflow-hidden">
              {buildings.map((building, index) => (
                <motion.div
                  ...
                  className="w-full max-w-sm bg-white rounded-2xl shadow-md border-2 border-gray-200 overflow-hidden">
                  <img
                    src={`${building.image}`}
                    alt="Campus"
                  ...
                  <div className="flex flex-col p-3">
                    ...
                    <button
                      onClick={() => setSelectedBuilding(building.buildingName)}
                      className="mt-4 w-full py-2 cursor-pointer duration-300 text-white bg-[#052e66] border-2 border-[#052e66] rounded-xl font-semibold shadow-md hover:bg-[#0b49a1] transition">
                      Ver Salas
                    </button>
                  </div>
                </motion.div>
        ...
        <ModifySalaModal
          ...
          onUpdated={getRooms}
          onRefresh={() => setUpdateModify(!updateModify)}
        />
          ...
          {selectedBuilding && (
            <motion.div
              ...
              <button
                ...
                className="w-fit text-[#052e66] font-semibold text-lg cursor-pointer transition-transform duration-300 hover:-translate-x-2">
                  ...
                  {!salas || salas.length === 0 ? (
                    ...
                    <>
                      ...
                      <ul className="w-full overflow-auto scrollbar mt-1 p-2">
                        {salas.map((data) => (
                          <li
                            key={data.studyRoomId}
                            className="bg-[#f4f7fc] border border-gray-200 hover:bg-[#e9eef7] transition-all duration-300 rounded-md flex lg:flex-row flex-col lg:items-center justify-between lg:h-20 mb-3 p-3 lg:gap-3 gap-2">
                            ...
                            <div className="lg:w-1/5 w-full lg:border-r-2 border-gray-300">
                              <p
                                className={`text-sm lg:text-lg text-gray-800 font-semibold break-all text-left lg:text-center ${
                                  data.status === 'Activo'
                                    ? 'text-green-600'
                                    : 'text-red-600'
                                }`}>
                            ...
                            <div className="lg:w-1/4 w-full lg:border-r-2 border-gray-300">
                              <div className="flex flex-wrap justify-start lg:justify-center gap-2">
                                <span className="lg:hidden inline">Tipo: </span>
                                {data.roomType}
                              </div>
                            </div>
