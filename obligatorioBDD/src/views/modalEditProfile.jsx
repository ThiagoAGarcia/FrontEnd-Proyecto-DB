...
export default function ModalEditProfile({ open, onClose, user }) {
    ...
    const validarFormulario = async () => {
        ...
        if (!lastName.trim() || !regexNombre.test(lastName) || lastName.trim().length < 3)
        ...
        if (changePass) {
            ...
            if (!newPassword) e.newPassword = "Ingrese una nueva contraseña";
                ...
                e.newPassword = "Mínimo 9 caracteres";

            if (!confirmPassword) e.confirmPassword = "Debe confirmar la nueva contraseña";
    ...
    return (
        <Modal open={open} onClose={isLoading ? () => { } : onClose}>
            <div className="relative max-h-[90vh] w-full p-4 sm:p-6 pr-8 rounded-xl overflow-y-auto">
                ...
                <h2 className="font-bold text-[#052e66] text-3xl mb-6">Editar perfil</h2>
                ...
                <div className="w-full bg-white shadow-md rounded-2xl p-5 border border-gray-300">
                    <section className="max-h-[60vh] overflow-y-auto scroll-ucu p-2">
                        ...
                        <div className="flex items-center gap-3 mt-4 mb-2">
                            <label className="font-medium text-[#052e66]">¿Cambiar contraseña?</label>

                            <button
                                type="button"
                                onClick={() => setChangePass(!changePass)}
                                className={`w-12 h-6 flex items-center rounded-full p-1 transition ${changePass ? "bg-[#052e66]" : "bg-gray-400"
                                    }`}
                            >
                                <div
                                    className={`bg-white w-5 h-5 rounded-full shadow-md transform transition ${changePass ? "translate-x-6" : "translate-x-0"
                                        }`}
                                />
                            </button>
                        </div>
                        ...
                        {changePass && (
                            <div className="space-y-3 mt-3 bg-gray-50 p-4 rounded-xl border">
                                ...
                                <div>
                                    ...
                                    />
                                    {errores.oldPassword && (
                                        <p className="text-red-600 text-xs">{errores.oldPassword}</p>
                                    )}
                                </div>
                                ...
                                <div>
                                    ...
                                    />
                                    {errores.newPassword && (
                                        <p className="text-red-600 text-xs">{errores.newPassword}</p>
                                    )}
                                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 mt-5 justify-end">
                    <button onClick={validarFormulario} disabled={isLoading} className="bg-[#052e66] cursor-pointer text-white w-full sm:w-1/3 py-3 rounded-xl shadow-md hover:bg-[#073c88] transition disabled:opacity-60">
                        Guardar cambios
                    </button>

                    <button onClick={onClose} disabled={isLoading} className="border border-[#052e66] cursor-pointer text-[#052e66] w-full sm:w-1/3 py-3 rounded-xl shadow-md hover:bg-[#eef3fb] transition">
                        Cancelar
                    </button>
                </div>
            </div>
